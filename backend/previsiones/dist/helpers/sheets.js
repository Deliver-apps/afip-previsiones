...
const calculateResult = (campos) => {
    const ventasIVA = stringToNumber(campos.ventas.operaciones.debito) -
        stringToNumber(campos.ventas.notasDeCredito.debito);
    const comprasIVA = stringToNumber(campos.compras.operaciones.debito) -
...
const formatMoneyFields = (campos) => {
    return {
        ventasNeto: moneyFormat(stringToNumber(campos.ventas.operaciones.neto) -
            stringToNumber(campos.ventas.notasDeCredito.neto)),
        ventasIVA: moneyFormat(stringToNumber(campos.ventas.operaciones.debito) -
            stringToNumber(campos.ventas.notasDeCredito.debito)),
        ventasTotal: moneyFormat(stringToNumber(campos.ventas.operaciones.neto) -
            stringToNumber(campos.ventas.notasDeCredito.neto) +
            stringToNumber(campos.ventas.operaciones.debito) -
            stringToNumber(campos.ventas.notasDeCredito.debito)),
        comprasNeto: moneyFormat(stringToNumber(campos.compras.operaciones.neto) -
            stringToNumber(campos.compras.notasDeCredito.neto)),
        comprasIVA: moneyFormat(stringToNumber(campos.compras.operaciones.debito) -
            stringToNumber(campos.compras.notasDeCredito.debito)),
        comprasTotal: moneyFormat(stringToNumber(campos.compras.operaciones.neto) -
            stringToNumber(campos.compras.notasDeCredito.neto) +
...
const putSheetData = async (data) => {
    logger_1.logger.debug("Putting data into Google Sheets");
    logger_1.logger.debug(data);
    if (!data || data.length === 0) {
        return;
    }
    const credentials = JSON.parse(config_1.config.secretClient ?? "");
    ...
    const spreadsheetId = config_1.config.sheetWebId;
    ...
    const transposedValues = allVerticalValues[0].map((_, colIndex) => allVerticalValues.map((row) => row[colIndex]));
    // Flatten each cell to ensure it's a simple string or number
    const flattenedTransposedValues = transposedValues.map((row) => row.map((cell) => (Array.isArray(cell) ? cell.join(" ") : cell)));
...
exports.putSheetData = putSheetData;
